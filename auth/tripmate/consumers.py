...
class NotificationConsumer(AsyncWebsocketConsumer):
    ...
    def get_pending_notifications_count(self):
        friend_requests_count = FriendRequest.objects.filter(receiver=self.user,status='pending').count()
        trip_shares_count = TripShare.objects.filter(shared_with=self.user,status='pending').count()
        return {'friend_requests': friend_requests_count,'trip_shares': trip_shares_count,'total': friend_requests_count + trip_shares_count}