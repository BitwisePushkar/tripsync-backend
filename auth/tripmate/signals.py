...
def friend_request_notification(sender, instance, created, **kwargs):
    if created and instance.status == 'pending':
        ...
        notification_data = {
            'id': instance.id,
            'responder_id': instance.receiver.id,
...
def trip_share_notification(sender, instance, created, **kwargs):
    if created and instance.status == 'pending':
        ...
        notification_data = {
            'id': instance.id,
            'trip_id': instance.itenary.id,
            'trip_name': instance.itenary.tripname,
            'sharer_id': instance.shared_by.id,
            'sharer_name': sharer_name,
            'role': instance.role,
            'message': instance.invitation_message,
            'created_at': instance.created_at.isoformat()
        }
        
        async_to_sync(channel_layer.group_send)(
            f'notifications_{instance.shared_with.id}',
            {
                'type': 'trip_share_notification',
                'data': notification_data
            }
        )
        ...
        notification_data = {
            'id': instance.id,
            'trip_name': instance.itenary.tripname,
            'responder_id': instance.shared_with.id,
        ...
        async_to_sync(channel_layer.group_send)(
            f'notifications_{instance.shared_by.id}',
            {
