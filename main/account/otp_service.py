...
def generate_and_store_otp(email: str, otp_type: Literal["registration", "password_reset"],) -> str:
    ...
    logger.info("OTP generated for %s (type=%s, resend_count=%d)", email, otp_type, resend_count)
    return otp_code
...
def verify_otp(email: str, otp_code: str, otp_type: Literal["registration", "password_reset"],) -> tuple[bool, str, int]:
    ...
    if attempts_used >= max_attempts:
        ...
        r.delete(_key_otp(email))
        logger.warning(
            "OTP lock triggered for %s after %d failed attempts", email, attempts_used
        )
        return (
            False,
            f"Too many failed attempts. Account locked for {lock_minutes} minute(s).",
            -1,
        )
    ...
    logger.warning(
        "Wrong OTP for %s — %d attempt(s) remaining", email, attempts_remaining
    )
...
def check_resend_eligibility(email: str) -> tuple[bool, str]:
    ...
    if raw:
        payload = json.loads(raw)
        if payload.get("resend_count", 0) >= settings.OTP_MAX_RESEND_ATTEMPTS:
            r.setex(
                _key_lock(email),
                settings.OTP_LOCK_DURATION_MINUTES * 60,
                "1",
            )
...
def store_unverified_user(email: str, username: str, password: str,) -> None:
    ...
    logger.debug("Unverified user data stored in Redis for %s", email)
...
def delete_unverified_user(email: str) -> None:
    ...
    logger.debug("Unverified user data cleared from Redis for %s", email)
...
def store_reset_token(email: str) -> str:
    ...
    r.setex(_key_reset_token(email), ttl, token_hash)
...
def record_failed_login(email: str) -> tuple[bool, int]:
    ...
    if attempts >= max_attempts:
        r.expire(key, lock_duration)
        logger.warning("Login lock triggered for %s", email)
        return True, 0
    remaining = max_attempts - attempts
    return False, remaining

def check_login_lock(email: str) -> tuple[bool, int]:
    r = get_redis()
    key = _key_login_lock(email)
    ttl = r.ttl(key)
    if ttl <= 0:
...
def clear_login_lock(email: str) -> None:
    r = get_redis()
    r.delete(_key_login_lock(email))
