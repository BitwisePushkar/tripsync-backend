...
class PostSerializer(serializers.ModelSerializer):
    ...
    img = serializers.ImageField(write_only=True, required=False, allow_null=True)
    vid = serializers.FileField(write_only=True, required=False, allow_null=True)
    ...
    class Meta:
        model = Post
        fields = [
            'id', 'user', 'title', 'desc', 'loc', 'rating',
            'img', 'vid', 'img_url', 'vid_url',
            'likes', 'dislikes', 'total_comments',
            'reaction', 'owner', 'created', 'updated'
        ]
    ...
    def validate_vid(self, val):
        if val:
            if val.size > 100 * 1024 * 1024:
                raise serializers.ValidationError("Video size must not exceed 100MB")
